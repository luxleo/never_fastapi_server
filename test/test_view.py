...
database = create_engine(settings.test_config["DB_URL"],encoding='utf-8',max_overflow=0)

@pytest.fixture
def api():
    return TestClient(create_app("test"))
...
def test_create_song(api):
    res = api.post('/create/song', json ={'title':'dragon4',"artist":"dragon","e_label":1,"v_label":2,"mood_tag":'평안한'})
    assert res.status_code == 200
    query = 'dragon4'
    res = api.get(f'/song/?q={query}')
    assert res.status_code ==200
...
def test_create_lyric_tag(api):
    res = api.post('/create/lyrictag',json={'tag_name':'test_tag'})
    assert res.status_code ==200

def test_create_map(api):
    res = api.post('/create/map',json={'song_id':1,'tag_id':3})
    assert res.status_code== 200